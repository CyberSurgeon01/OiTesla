...
export default function DriverHistory() {
  ...
  return (
    <div className="min-h-screen bg-[#131815] text-[#F3F4F6] font-sans selection:bg-[#10B981]/30 pb-20">
      <header className="sticky top-0 z-40 w-full bg-[#131815]/90 backdrop-blur-xl border-b border-[#3E4C44]">
        <div className="max-w-screen-xl mx-auto flex h-16 items-center px-6 gap-4">
          <Link href="/driver/dashboard" className="p-2 -ml-2 rounded-full hover:bg-[#1E2621] transition-colors text-[#D1D5DB] hover:text-[#F3F4F6]">
      ...
      <main className="max-w-lg mx-auto p-6 space-y-6">
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-[#3F3F46] p-12 text-center text-[#D1D5DB] bg-[#1E2621]">
          ...
          <div className="space-y-4">
            {history.map((pool: any) => {
              ...
              return (
                <div key={pool.id} className="bg-[#1E2621] border border-[#3E4C44] rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow">
                  <div className="p-5 border-b border-[#3E4C44] flex justify-between items-center bg-[#2C3831]">
                  ...
                  <div className="p-5">
                    <div className="flex justify-between items-end mb-6">
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-[#D1D5DB] mb-1">Total Earnings</span>
                        <span className="font-semibold text-3xl text-[#F3F4F6]">
                          ৳{(totalFare / 100).toFixed(2)}
                        </span>
                      </div>
                      <div className="text-sm font-medium text-[#D1D5DB] bg-[#131815] border border-[#3E4C44] px-3 py-1 rounded-full mb-1 flex items-center gap-1.5">
                        <User className="w-4 h-4" /> {totalSeats} Passenger{totalSeats > 1 ? 's' : ''}
                      </div>
                    </div>

                    <div className="space-y-3">
                      <span className="text-xs font-semibold uppercase tracking-widest text-[#D1D5DB]">Trip Details</span>
                      <div className="divide-y divide-[#2C3831] border border-[#3E4C44] rounded-xl overflow-hidden">
                        {pool.rideRequests.map((ride: any) => (
                          <div key={ride.id} className="p-3 bg-[#131815] flex justify-between items-center">
                            <div className="flex flex-col">
                              <span className="text-sm font-medium text-[#F3F4F6]">{ride.passenger.name}</span>
                              <span className="text-xs text-[#D1D5DB] mt-0.5">{ride.pickup_zone} → {ride.destination_zone}</span>
                            </div>
