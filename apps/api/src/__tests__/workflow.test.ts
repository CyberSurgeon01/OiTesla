...
describe('Workflow & Rules Enforcement', () => {
  ...
  beforeAll(async () => {
    ...
    const driver = await prisma.user.create({ data: { email: 'jashim_test@oitesla.com', name: 'Jashim', password_hash, role: Role.DRIVER } });
    driverToken = jwt.sign({ id: driver.id, email: driver.email, role: driver.role }, JWT_SECRET);

    await prisma.vehicle.create({ data: { driver_id: driver.id, name: 'Bullet', seat_capacity: 3, status: VehicleStatus.ONLINE } });
    
    const p1 = await prisma.user.create({ data: { email: 'nusrat_test@oitesla.com', name: 'Nusrat', password_hash, role: Role.PASSENGER } });
    passenger1Token = jwt.sign({ id: p1.id, email: p1.email, role: p1.role }, JWT_SECRET);

    const p2 = await prisma.user.create({ data: { email: 'rafiq_test@oitesla.com', name: 'Rafiq', password_hash, role: Role.PASSENGER } });
  ...
  it('allows Nusrat to request a ride', async () => {
  ...
  it('data isolation: Rafiq cannot view or modify Nusrat ride', async () => {
    // Passenger 2 tries to cancel Nusrat's ride
