...
describe('Concurrency & Capacity Enforcement', () => {
  ...
  it('seat capacity can never be exceeded (simulate Nusrat vs Shirin for the last seat)', async () => {
    // First, fill 2 seats using a rafiq passenger to leave exactly 1 seat on Bullet
    const rafiq = await prisma.user.create({
      data: { email: 'rafiq_test@oitesla.com', name: 'Rafiq', password_hash: 'x', role: Role.PASSENGER }
    });
    const rafiqToken = jwt.sign({ id: rafiq.id, email: rafiq.email, role: rafiq.role }, JWT_SECRET);
    ...
    expect(totalUsed).toBe(3); // 2 from rafiq + 1 from the winner
  });
});
