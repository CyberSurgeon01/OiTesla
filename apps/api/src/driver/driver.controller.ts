...
export const transitionPool = async (req: any, res: Response) => {
  try {
    ...
    if (!pool) return res.status(404).json({ error: 'Pool not found for this vehicle' });
    if (pool.status === PoolStatus.COMPLETED) return res.status(400).json({ error: 'Out of order transition: Pool is already COMPLETED' });
